...
/obj/machinery/atmospherics/omni/filter/error_check()
	if(!input || !output || !omni_filters)
		return 1
	if(omni_filters.len < 1) //requires at least 1 filter ~otherwise why are you using a filter?
		return 1
...
/obj/machinery/atmospherics/omni/filter/process()
	...
	for(var/datum/omni_port/filter_output in omni_filters)
	...
	if (power_draw >= 0)
		...
		for(var/datum/omni_port/P in omni_filters)
			if(P.network)
				P.network.update = 1
