...
/obj/item/mecha_parts/mecha_equipment/tool/sleeper/Topic(href,href_list)
	...
	if(lfilter.get("eject"))
		go_out()
	if(lfilter.get("view_stats"))
		chassis.occupant << browse(get_occupant_stats(),"window=msleeper")
		onclose(chassis.occupant, "msleeper")
		return
	if(lfilter.get("inject"))
		inject_reagent(lfilter.getType("inject",/datum/reagent),lfilter.getObj("source"))
	return
...
/obj/item/mecha_parts/mecha_equipment/tool/syringe_gun/Topic(href,href_list)
	..()
	var/datum/topic_input/lfilter = new (href,href_list)
	if(lfilter.get("toggle_mode"))
		mode = !mode
		update_equip_info()
		return
	if(lfilter.get("select_reagents"))
		...
		for(var/i=1 to known_reagents.len)
			if(m>=synth_speed)
				break
			var/reagent = lfilter.get("reagent_[i]")
		...
		return
	if(lfilter.get("show_reagents"))
		chassis.occupant << browse(get_reagents_page(),"window=msyringegun")
	if(lfilter.get("purge_reagent"))
		var/datum/reagent/R = locate(lfilter.get("purge_reagent")) in reagents.reagent_list
		if(R)
			reagents.del_reagent(R.type)
		return
	if(lfilter.get("purge_all"))
